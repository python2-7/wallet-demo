...
@IonicPage()
@Component({
  selector: 'page-share-commission',
  templateUrl: 'share-commission.html',
})
export class ShareCommissionPage {
	customer_id : any;
	count_history : any;
	history : any;
	history_load_no : any;
	...
	ionViewDidLoad() {
		...
		this.AccountServer.GetHisroryProfit(this.customer_id,'Share commission',0,10)
        .subscribe((data) => {
        	loading.dismiss();
			if (data)
			{
	...
	ionViewWillEnter() {
  	...
  	doInfinite(infiniteScroll : InfiniteScroll) {
		...
		this.AccountServer.GetHisroryProfit(this.customer_id,'Share commission',this.history.length,5)
        .subscribe((data) => {
        	if (data.length > 0)
			{
